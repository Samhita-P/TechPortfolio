...
export function DevTerminal() {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key.length === 1 || e.key === "Backspace" || e.key === " ") {
        buf.current = (e.key === "Backspace" ? buf.current.slice(0, -1) : buf.current + e.key).slice(-32);
        if (buf.current.toLowerCase().includes("sudo hire samhita")) {
          setGranted(true);
          buf.current = "";
          setTimeout(() => setGranted(false), 4000);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setFeed((f) => [...f, `${new Date().toISOString().slice(11, 19)}  ${fakeLogs[i % fakeLogs.length]}`].slice(-14));
      i++;
    }, 350);
        ...
        {open && (
          ...
          >
            <div className="flex items-center gap-2 px-4 py-2 border-b border-primary/20 bg-primary/5">
              <span className="size-2 rounded-full bg-destructive/80 cursor-pointer" onClick={() => setOpen(false)} />
              <span className="size-2 rounded-full bg-yellow-400/70" />
              <span className="size-2 rounded-full bg-primary" />
              <span className="ml-2 text-[10px] tracking-[0.3em] text-muted-foreground">samhita@cortex · /dev/console</span>
              <span className="ml-auto text-[10px] tracking-[0.3em] text-primary">DEV MODE</span>
            </div>
            ...
            <div className="p-4 h-56 overflow-hidden text-foreground/85 leading-relaxed">
              {feed.map((l, i) => (
                <div key={i} className="opacity-90"><span className="text-primary">›</span> {l}</div>
              ))}
              <div className="text-primary cursor-blink">&nbsp;</div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {granted && (
          ...
          >
            <div className="glass-strong rounded-2xl px-10 py-8 text-center">
              <div className="font-mono text-[10px] tracking-[0.4em] text-primary">SECURITY · OVERRIDE</div>
              <div className="mt-3 font-display text-4xl text-gradient glow-text">ACCESS GRANTED.</div>
              <div className="mt-2 text-xs text-muted-foreground font-mono">welcome, recruiter ✦</div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function Metric({ k, v }: { k: string; v: string }) {
  return (
    <div className="rounded-md border border-primary/20 bg-background/40 px-2 py-1.5 text-center">
      ...
      <div className="text-primary text-sm">{v}</div>
    </div>
  );
}